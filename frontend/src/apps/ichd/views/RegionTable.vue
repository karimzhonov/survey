<template>
    <div v-if="loading" class="layout-main row justify-content-center">
      ...
      <div v-if="!loading">
        <BackOrStart :header="`${$t('Районы')} (${$t(criteria.translations[$i18n.locale].name)})`" :navigator="[]" :home_to="{name: 'ichd_main', query: this.$route.query}"/>
...
export default {
    ...
    async mounted() {
        ...
        this.data = await ichd.region.table.list({upload: this.$route.query.file, category: this.$route.query.criteria})
