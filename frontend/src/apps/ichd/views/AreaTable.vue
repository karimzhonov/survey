  ...
  <div v-if="loading" class="layout-main row justify-content-center">
    ...
    <div v-if="!loading">
      <BackOrStart :header="criteria.translations[$i18n.locale].name" :navigator="[
                {label: region.translations[$i18n.locale].name, to: {name: 'ichd_sector_id_region_id', params: {sector_id: sector_id, region_id: region_id}, query: $route.query}},
                  ...
                  <DataTable ref="dt" :value="data" showGridlines :responsiveLayout="width < 960 ? 'stack': 'scroll'">
                    <template #header>
                      ...
                      <Column v-for="(col, i) in criteriaes" :key="i" :field="col.name" :header="width < 960 ? $t(col.name) : null">
                        ...
                        <template #body="slotProps">
                      <router-link v-if="has_criteria_data(slotProps.data, col)" :to="{name: 'ichd_area-criteria-data-table', params:{area_id: slotProps.data.area.id}, query: {...this.$route.query, inner_criteria: col.id, region: this.region_id}}">
...
export default {
  ...
  async mounted() {
      this.criteria = await ichd.criteria.get(this.criteria_id)
      const data = await ichd.area.table.list({upload: this.$route.query.file, category: this.criteria_id, region: this.region_id, sector: this.sector_id})
      const d = {}
      ...
      this.criteriaes = await ichd.criteria.list({category: this.criteria_id})
