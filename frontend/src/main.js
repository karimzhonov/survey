...
export const i18n = createI18n({
  locale: window.location.pathname.split('/')[1] === 'frame' ? window.location.pathname.split('/')[2] : undefined,
  legacy: false,
  messages: locale,
});

router.afterEach((to, parent) => {
  store.commit('basic', {key: 'loading', value: true})
  if (parent.path.startsWith('/frame') && to.name !== parent.name) {
    return router.push({name: to.name.startsWith('frame_') ? to.name : `frame_${to.name}`, query: to.query, params: to.params});
  }
  setTimeout(() => {
